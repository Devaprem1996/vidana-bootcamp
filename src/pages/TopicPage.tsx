      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-[#1A1D24] p-6 rounded-[2rem] border border-gray-100 dark:border-gray-800 shadow-sm hover:shadow-md transition-all group">
             <div className="flex justify-between items-start mb-4">
                ...
                <span className="text-xs font-bold uppercase text-gray-400 tracking-wider">Status</span>
            </div>
            <div className="text-3xl font-heading font-bold text-gray-900 dark:text-white mb-1">
                {completedDays.length === curriculum.length && curriculum.length > 0 ? 'Certified' : 'Active'}
            </div>
            <div className="text-xs font-medium text-gray-500 dark:text-gray-400">Certification</div>
        </div>
            ...
            {curriculum.map((day) => {
                ...
                return (
                    <div key={day.day} className="flex flex-col items-center relative z-10 group cursor-pointer" onClick={() => setActiveTab('learning')}>
                        <div className={`w-11 h-11 rounded-2xl flex items-center justify-center border-4 transition-all duration-300 shadow-sm ${
                            isDone 
  ...
  const LearningTab = () => (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        {curriculum.map((day) => {
            const isDone = completedDays.includes(day.day);
            return (
                <div key={day.day} className={`bg-white dark:bg-[#1A1D24] border rounded-[2rem] overflow-hidden transition-all duration-300 ${
                    expandedDay === day.day ? 'shadow-lg border-brand-200 dark:border-brand-900/50 ring-1 ring-brand-100 dark:ring-brand-900/30' : 'shadow-sm border-gray-100 dark:border-gray-800 hover:border-gray-200 dark:hover:border-gray-700'
                }`}>
                    <div 
                    ...
                    >
                        <div className="flex items-center space-x-3 md:space-x-5 flex-1 min-w-0">
                            <div className={`w-10 h-10 md:w-14 md:h-14 rounded-xl shrink-0 flex items-center justify-center font-bold text-sm md:text-lg transition-colors shadow-sm ${
                                isDone ? 'bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400' : 'bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 text-gray-400'
                            }`}>
                        ...
                        <div className="p-4 md:p-8 border-t border-gray-100 dark:border-gray-800 space-y-6 md:space-y-8 animate-in fade-in">
                            ...
                            <div className="flex flex-col sm:flex-row justify-between items-center gap-4 pt-4 border-t border-gray-100 dark:border-gray-800">
                                ...
                                <button 
                                    ...
                                    className={`w-full sm:w-auto px-8 py-3 rounded-xl text-sm font-bold transition-all shadow-lg hover:shadow-xl hover:-translate-y-0.5 ${
                                        isDone 
                    ...
                    {[
                        ...
                        <label key={i} className="flex items-center space-x-4 p-4 rounded-2xl border border-transparent hover:border-gray-200 dark:hover:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800/50 cursor-pointer transition-all group">
                            <div className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-colors ${
                                getChecklist().includes(task) ? 'bg-purple-500 border-purple-500' : 'border-gray-300 dark:border-gray-600'
                            }`}>
      ...
      <div className="space-y-8 animate-in fade-in">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-white dark:bg-[#1A1D24] p-8 rounded-[2rem] border border-gray-100 dark:border-gray-800 shadow-sm">
                  <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-6 flex items-center"><Star className="mr-2 text-yellow-500" size={20}/> Grading Rubric</h3>
                  <div className="space-y-4">
                      {[{l:'Excellent (3pts)', d:'Perfect execution, clean logic.'}, {l:'Good (2pts)', d:'Functional but needs cleanup.'}, {l:'Needs Work (1pt)', d:'Failed execution.'}].map((r,i) => (
                          <div key={i} className="p-4 rounded-2xl bg-gray-50 dark:bg-gray-800/50">
                              <div className="text-sm font-bold text-gray-900 dark:text-white">{r.l}</div>
                              <div className="text-xs text-gray-500 dark:text-gray-400">{r.d}</div>
                          </div>
                      ))}
                  </div>
              </div>
               <div className="bg-white dark:bg-[#1A1D24] p-8 rounded-[2rem] border border-gray-100 dark:border-gray-800 shadow-sm flex flex-col items-center justify-center text-center">
                  <div className="w-20 h-20 bg-brand-50 dark:bg-brand-900/20 rounded-full flex items-center justify-center mb-6 text-brand-600 dark:text-brand-400">
                      <GraduationCap size={32} />
                  </div>
                  <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">Your Gradebook</h3>
                  <p className="text-gray-500 text-sm mb-6">No grades recorded yet.</p>
                  <button className="px-6 py-2 bg-gray-900 dark:bg-white text-white dark:text-black rounded-xl text-sm font-bold">Download Report</button>
              </div>
          </div>
      </div>
  );
    ...
    <div className="max-w-7xl mx-auto pb-20">
      ...
      <div className="sticky top-[64px] md:top-4 z-30 mb-6 md:mb-10 mx-auto max-w-full md:max-w-fit transition-all">
        <nav className="flex items-center space-x-1 p-1.5 bg-white/90 dark:bg-[#151515]/90 backdrop-blur-xl border border-gray-200/50 dark:border-white/10 rounded-xl md:rounded-2xl shadow-lg shadow-black/5 overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none'] w-full md:w-auto">
            {(['dashboard', 'learning', 'workbook', 'resources', 'assessment', 'setup'] as Tab[]).map((tab) => (
                <button
                    key={tab}
